...
function saveComment(_text,_shareid,_obj){
	...
	$.ajax({
		...
		timeout: 10000		
			...
			_tar_count.text('回复('+commentcount+')');
			}
			setCookie('pub_content',_text,'h24');
			if(pub_time==null){
				setCookie('pub_count',1,'h24');
...
function shareList(jsonData,i,zhuan){
	...
	if(jsonData.items[i].sharecount==0)
	listTemplate+='<div class="span3">新私信</div>';
	listTemplate+='<div class="span6 pull-right text-right">';
	if(jsonData.staff || jsonData.items[i].dingcount==0 )
	...
	listTemplate+='<a onClick="togglecomment(this)" href="javascript:void(0);">回复</a>';
	listTemplate+='</div>';
	listTemplate+='</div>';
...
function showcomment(_obj){	
	...
	$.ajax({
		...
		timeout: 10000		
			...
			template='<textarea rows="1" class="comment-text"  title="回复内容" placeholder="请输入回复内容"></textarea>';
			...
			template+='<div class="span4 text-left"></div>';
			template+='<div class="span2 offset4 text-right" style="float:right;"><button type="button" class="btn btn-small btn-warning comment-btn">回复</button></div>';
			template+='</div>';			
			_tar.html(template);
