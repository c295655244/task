...
class DomainMapper:
	...
	def lookup(self, domain):
		domain = domain.lower()#将所有大写转换成小写
...
class CustomResolver(client.Resolver):
	...
	def lookupAddress(self, name, timeout=None):
		...
		if kname is not None:
			...
			def packResult(value):
				return [
					(dns.RRHeader("127.0.0.1", dns.A, dns.IN, self.config.hosts_ttl, dns.Record_A(value, self.config.hosts_ttl)),),
					(),
					(),
				]
			print "此处fail"
