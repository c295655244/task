...
def monitorone(domain): 
    ...
    try:
        ...
        for ans in rsp.answers:
            if ans['data']=="123.129.254.15"  or  ans['data']=="221.192.153.42":
                break
            sql="select * from domain_a_address where domain=\"%s\" and ip=\"%s\"" %(domain,ans['data'])
            n=cursor.execute(sql)
            if n:
                pass
            else:                
                sql="INSERT INTO domain_a_address (domain,ip) VALUES(\"%s\",\"%s\")" %(domain,ans['data'])
                print domain+"------"+ans['data']+"-----------write!"
                cursor.execute(sql)
                conn.commit()
    except Exception, ex:
            print domain+"------"+str(ex)
            sql="INSERT INTO domain_error(domain,reason) VALUES(\"%s\",\"%s\")" %(domain,str(ex))
            cursor.execute(sql)
            conn.commit()
    finally:
        lock.release()
...
if __name__ == '__main__':
    ...
    list = [spawn(monitorone, domain.strip()) 
        for domain in open('./domains1.txt')]
    joinall(list)
